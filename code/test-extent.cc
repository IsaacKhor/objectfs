...
struct buf_extent { // logical extent -> buffer
  public:
    ...
    {
        return ((a.limit == b.base) && (a.buf + b.base - a.base) == b.buf);
    }
    ...
    void new_limit(int64_t _limit) { limit = _limit; }
};
...
struct plain_extent { // logical extent. period.
  public:
    ...
    void new_base(int64_t _base) { base = _base; }
    void new_limit(int64_t _limit) { limit = _limit; }
};

void merge_extents(std::vector<offset_extent> &in_exts, char *in_buf,
                   std::vector<offset_extent> &out_exts, char *out_buf)
{
    ...
    for (auto it = in_exts.begin(); it != in_exts.end(); it++) {
        map1.update(
            (buf_extent){.base = it->base, .limit = it->limit, .buf = in_buf});
    ...
    for (auto e = map2.first(); e != nullptr; e = map2.next(e)) {
        out_exts.push_back((offset_extent){
            .base = e->base, .limit = e->limit, .offset = offset});
