...
template <class P, class M> P *my_container_of_impl(M *ptr, const M P::*member)
{
    return (P *)((char *)ptr - my_offsetof(member));
}
#define container_of(ptr, type, member) my_container_of_impl(ptr, &type::member)

template <class T> struct extmap {
    class _extent
    {
      public:
        struct rb_node rb;
  ...
  public:
    ...
    void update(T _e) { _update(_e, false); }
    void trim(T _e) { _update(_e, true); }
